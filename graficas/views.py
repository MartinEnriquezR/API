...
class graficasViewSet(viewsets.GenericViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='edad-alertas')
    ...
    @action(detail=False, methods=['post'], url_path='alertas-mes')
    ...
    @action(detail=False, methods=['post'], url_path='horas-alertas-mes')
    ...
    @action(detail=False, methods=['post'], url_path='denuncias-previas')
    ...
    @action(detail=False, methods=['post'], url_path='alertas-nacionalidad')
    ...
    @action(detail=False, methods=['post'], url_path='alertas-complexion')
    ...
    @action(detail=False, methods=['post'], url_path='alertas-color-ojos')
    ...
    @action(detail=False, methods=['post'], url_path='alertas-color-piel')
    ...
    #incidencias totales por año
    @action(detail=False, methods=['post'], url_path='incidencias-totales')
    def incidenciasAno(self,request,*args,**kwargs):
        #ano del cual se quieren ver las incidencias
        serializer = incidenciasTotalesSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.save()

        return Response(data,status=status.HTTP_200_OK)

    #puntos geograficos donde empezo la alerta
    @action(detail=False, methods=['post'],url_path='puntos-incidencias')
    def puntosIncidencias(self,request,*args,**kwargs):
        #año en el que se quieren visualizar los puntos de incidencias
        #solo se debe de incluir el punto donde comenzaron las incidencias

        serializer = puntosIncidenciasSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.save()
